...
def train(dataset, model, loss, optimazer, epochs):
    ...
    for epoch in range(epochs):
        for step, batch in enumerate(tqdm(train_dataloader)):
            ...
            l.backward()
            optimazer.step()
        l = dev(model, loss, dev_dataloader)
...
def main(args):
    ...
    model = Model(args.input_dim, args.output_dim)
    loss = nn.MSELoss()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_dim", default=None, type=float, required=True, help="output_dim")
